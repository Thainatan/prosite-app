...
function DotsMenu({ onView, onDuplicate, onArchive, onDelete }: { onView: () => void; onDuplicate: () => void; onArchive: () => void; onDelete: () => void }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={e => { e.stopPropagation(); setOpen(o => !o); }} className="w-8 h-8 rounded-full flex items-center justify-center text-[#3D4466] hover:bg-[#1E2130] transition-colors text-[18px] font-bold">⋯</button>
      {open && (
        <div className="absolute right-0 top-9 z-50 bg-[#0F1117] border border-[#1E2130] rounded-[10px] shadow-[0_8px_32px_rgba(0,0,0,0.4)] w-40 overflow-hidden" onClick={e => e.stopPropagation()}>
          {[
            { label: 'View / Print', action: () => { onView(); setOpen(false); }, color: '#E2E8F0' },
            { label: 'Duplicate', action: () => { onDuplicate(); setOpen(false); }, color: '#E2E8F0' },
            { label: 'Archive', action: () => { onArchive(); setOpen(false); }, color: '#F5A623' },
            { label: 'Delete', action: () => { onDelete(); setOpen(false); }, color: '#F0584C' },
          ].map(({ label, action, color }) => (
            <button key={label} onClick={action} className="w-full text-left px-4 py-2.5 text-[13px] font-medium hover:bg-[#161924] border-b border-[#1E2130] last:border-0 transition-colors" style={{ color }}>
              {label}
            </button>
          ))}
        </div>
...
function PdfPreview({ quote, settings, onClose }: { quote: Quote; settings: CompanySettings | null; onClose: () => void }) {
  ...
  const brand = settings?.brandColor || '#4F7EF7';
      ...
      <div className="bg-white rounded-[16px] w-full max-w-3xl max-h-[92vh] flex flex-col shadow-[0_32px_80px_rgba(0,0,0,0.3)]" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#EAECF2]">
          <div className="flex items-center gap-3">
            <h2 className="text-[16px] font-bold text-[#1A1D2E]">{docLabel} Preview</h2>
          ...
          <div id="pdf-print-area" className="bg-white rounded-[12px] shadow-sm p-8" style={{ minHeight: 600 }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:28, paddingBottom:20, borderBottom:`2px solid ${brand}` }}>
              <div>
                ...
                <div style={{ fontSize:16, fontWeight:800, color:'#1A1D2E' }}>{settings?.companyName || 'Your Company'}</div>
              ...
              <div style={{ textAlign:'right' }}>
                ...
                <div style={{ fontSize:13, fontWeight:700, color:'#1A1D2E', marginTop:4 }}>{quote.estimateNumber}</div>
            ...
            <div style={{ marginBottom:24 }}>
              ...
              {quote.client ? (
                <>
                  <div style={{ fontSize:14, fontWeight:700, color:'#1A1D2E' }}>{quote.client.firstName} {quote.client.lastName}</div>
            ...
            <div style={{ marginBottom:24, display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
              <div style={{ background:'#F9FAFB', borderRadius:8, padding:'10px 14px' }}>
                ...
                <div style={{ fontSize:13, fontWeight:600, color:'#1A1D2E' }}>{quote.title}</div>
                    ...
                    return (
                      <tr key={i} style={{ borderBottom:'1px solid #F0F0F0' }}>
                        <td style={{ padding:'10px 12px' }}>
                          ...
                          <div style={{ fontSize:13, color:'#1A1D2E' }}>{item.description}</div>
                        </td>
                        ...
                        {hasLT && <td style={{ padding:'10px 12px', textAlign:'right', fontWeight:600, color:'#1A1D2E', fontSize:13 }}>{fmt(lineTotal)}</td>}
                      </tr>
                    );
                  })}
                </tbody>
...
export default function QuotesPage() {
  ...
  useEffect(() => {
    fetch(`${API}/quotes`).then(r => r.json()).then(data => { if (Array.isArray(data)) setQuotes(data); setLoading(false); }).catch(() => setLoading(false));
  ...
  return (
    <div className="min-h-screen bg-[#0A0D14]">
      ...
      <header className="bg-[#0F1117] border-b border-[#1E2130] h-14 flex items-center justify-between px-6 gap-3">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder={`Search ${docLabel.toLowerCase()}...`} className="flex-1 max-w-sm h-[34px] bg-[#161924] border border-[#1E2130] rounded-full px-4 text-[13px] text-white placeholder-[#3D4466] outline-none focus:border-[#4F7EF7] transition-all"/>
        <a href="/quotes/new" style={{display:'flex',alignItems:'center',gap:6,height:34,padding:'0 16px',background:'#4F7EF7',color:'white',fontSize:13,fontWeight:700,borderRadius:9,textDecoration:'none'}}>+ New {settings?.useEstimate ? 'Estimate' : 'Quote'}</a>
      </header>

      <div className="p-5">
        {loading ? (
          <div className="text-center py-12"><p className="text-[#8892B0]">Loading...</p></div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-[14px] font-semibold text-white mb-2">{search ? 'No results' : `No ${docLabel.toLowerCase()} yet`}</p>
            <a href="/quotes/new" style={{display:'inline-flex',alignItems:'center',padding:'10px 24px',background:'#4F7EF7',color:'white',borderRadius:9,fontSize:14,fontWeight:600,textDecoration:'none'}}>+ New {settings?.useEstimate ? 'Estimate' : 'Quote'}</a>
          </div>
        ) : (
          <div className="bg-[#0F1117] rounded-[14px] border border-[#1E2130] overflow-hidden">
            {filtered.map(q => {
              ...
              return (
                <div key={q.id} className="flex items-center gap-3 px-5 py-4 hover:bg-[#161924] border-b border-[#1E2130] last:border-0 transition-colors">
                  ...
                  {q.status === 'APPROVED' && (
                    <a href="/projects" className="flex-shrink-0 h-8 px-3 rounded-[8px] text-[11.5px] font-bold no-underline flex items-center" style={{ background: '#0C1A3A', color: '#4F7EF7', border: '1px solid #4F7EF744' }}>
                      View Project
                    </a>
                  )}
                  <DotsMenu
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setConfirmDelete(null)}>
          <div className="bg-[#0F1117] border border-[#1E2130] rounded-[16px] p-6 w-full max-w-sm shadow-xl" onClick={e => e.stopPropagation()}>
            <h3 className="text-[16px] font-bold text-white mb-2">Delete Quote?</h3>
            <p className="text-[13px] text-[#8892B0] mb-5">"{confirmDelete.title}" will be permanently deleted. This cannot be undone.</p>
            <div className="flex gap-2">
              <button onClick={() => setConfirmDelete(null)} className="flex-1 h-10 rounded-[9px] border border-[#1E2130] text-[#8892B0] text-[13px] font-semibold">Cancel</button>
