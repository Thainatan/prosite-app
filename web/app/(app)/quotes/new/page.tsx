...
function SubPicker({ subs, onSelect, onClose }: {
  ...
  return (
    <div ref={ref} className="absolute right-0 top-9 z-50 w-56 bg-[#0F1117] border border-[#1E2130] rounded-[10px] shadow-[0_8px_32px_rgba(0,0,0,0.4)] overflow-hidden">
      <div className="p-2 border-b border-[#1E2130]">
        <input
          autoFocus
          value={q} onChange={e => setQ(e.target.value)}
          placeholder="Search subs..."
          className="w-full h-7 bg-[#0A0D14] border border-[#1E2130] rounded-[7px] px-2 text-[12px] text-white placeholder-[#3D4466] outline-none"
        />
      </div>
      <div className="max-h-48 overflow-y-auto">
        {filtered.length === 0 ? (
          <p className="text-[11px] text-[#3D4466] text-center py-3">No subs found</p>
        ) : filtered.map(s => (
          <button key={s.id} onClick={() => { onSelect(s); onClose(); }}
            className="w-full text-left px-3 py-2 hover:bg-[#1E2130] border-b border-[#1E2130] last:border-0 flex items-center justify-between gap-2">
        ...
        <div className="bg-[#0F1117] rounded-[14px] border border-[#1E2130] overflow-hidden">
          ...
          <div className="grid grid-cols-12 gap-2 px-5 py-2.5 bg-[#161924] border-b border-[#1E2130]">
            {[
              ...
              <div key={label} className={`text-[10.5px] font-bold text-[#3D4466] uppercase ${cls}`}>{label}</div>
            ))}
          </div>

          {items.map(item => (
            <div key={item.id} className="border-b border-[#1E2130] hover:bg-[#161924]">
              {/* Main row */}
              <div className="grid grid-cols-12 gap-2 px-5 py-2.5 items-center">
                <div className="col-span-2">
                  <select value={item.section} onChange={e => updateItem(item.id,'section',e.target.value)}
                    className="w-full h-8 bg-[#0A0D14] border border-[#1E2130] rounded-[7px] px-2 text-[11.5px] text-[#8892B0] outline-none">
                    {SECTIONS.map(s => <option key={s}>{s}</option>)}
                  </select>
                </div>
                <div className="col-span-3">
                  <input value={item.description} onChange={e => updateItem(item.id,'description',e.target.value)}
                    placeholder="Description..." className="w-full h-8 bg-[#0A0D14] border border-[#1E2130] rounded-[7px] px-2 text-[12.5px] text-white placeholder-[#3D4466] outline-none"/>
                </div>
                <div className="col-span-1">
                  <input type="number" value={item.qty} onChange={e => updateItem(item.id,'qty',parseFloat(e.target.value)||0)}
                    className="w-full h-8 bg-[#0A0D14] border border-[#1E2130] rounded-[7px] px-2 text-[12.5px] text-white outline-none text-center"/>
                </div>
                <div className="col-span-1">
                  <select value={item.unit} onChange={e => updateItem(item.id,'unit',e.target.value)}
                    className="w-full h-8 bg-[#0A0D14] border border-[#1E2130] rounded-[7px] px-1 text-[11.5px] text-[#8892B0] outline-none">
                ...
                <div className="col-span-2">
                  <div className="relative">
                    ...
                    <input type="number" value={item.price} onChange={e => updateItem(item.id,'price',parseFloat(e.target.value)||0)}
                      className="w-full h-8 bg-[#0A0D14] border border-[#1E2130] rounded-[7px] pl-5 pr-2 text-[12.5px] text-white outline-none text-right"/>
                  </div>
                </div>
                    ...
                    <button
                      ...
                      className={`w-7 h-7 rounded-[6px] flex items-center justify-center transition-colors ${
                        item.subcontractorId
                          ? 'bg-[#1E2A4A] text-[#4F7EF7]'
                          : 'hover:bg-[#1E2130] text-[#3D4466] hover:text-[#8892B0]'
                      }`}>
                      <UserPlus size={12}/>
                    </button>
              ...
              {item.subcontractorId && (
                <div className="px-5 pb-2 flex items-center gap-1.5">
                  <span className="text-[10px] text-[#3D4466] font-semibold">Sub:</span>
                  ...
                  <button onClick={() => clearSub(item.id)} className="text-[#3D4466] hover:text-[#F0584C] transition-colors">
                    <X size={11}/>
                  </button>
                </div>
              )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Additional scope details..."
                className="w-full h-24 bg-[#0A0D14] border border-[#1E2130] rounded-[9px] px-3 py-2.5 text-[13px] text-white placeholder-[#3D4466] outline-none resize-none"/>
            </div>
            <div>
